...
class WpProQuiz_View_FrontQuiz extends WpProQuiz_View_View {
	...
	private function showStartQuizBox() {
		...
		<div class="wpProQuiz_text">
            ...
            <?php if ( ! $this->quiz->isBtnRestartQuizHidden() ) { ?>
					...
					<input style="display: none" id="custom_reset" class="wpProQuiz_button wpProQuiz_button_restartQuiz" type="button" name="restartQuiz"
					       value="<?php
							echo esc_html( SFWD_LMS::get_template(
								'learndash_quiz_messages',
								array(
									...
									'message' 		=> 	sprintf( esc_html_x( 'Start %s from beginning', 'Restart Quiz Button Label', 'learndash' ), LearnDash_Custom_Label::get_label( 'quiz' ) )
								)
							));
							?>">
	...
	private function showResultBox( $result, $questionCount ) {
		...
		<div class="discount-ad mb-4" style="display:none">
		<h2 class="h4">Earn a 20% Discount on Your Next Certification Course!</h2>
                <p class="mb-0">ACLScertification.org is proud to offer a 20% discount to students who successfully pass 5 of our practice tests in one sitting. Currently, you have passed <strong class="passedPracticeExamCount">0</strong> of the <strong>5</strong> required tests. Start testing your skills and earning your discount now in our <strong><a href="javascript:;">Practice Exam Section</a>.</strong></p>
            </div>
		<?php
	}
	...
	public function showQuizBox( $questionCount ) {
		...
		$quiz_post_id = $this->quiz->getID();
		if ( isset( $cookie_data[ $quiz_post_id ] ) ) {
			$saved_answers = trim($cookie_data[ $quiz_post_id ]['cookie']);
			$last_q_id = trim($cookie_data[ $quiz_post_id ]['last_q_id']);
			if ( ! empty( $saved_answers ) ) {
				$saved_answers = \LDTSQ\Json::decode( html_entity_decode($saved_answers), true );
				?>
                <script>var saved_cookie_value = <?php echo \LDTSQ\Json::encode( $saved_answers );?>;
                var last_q_id = '<?php echo $last_q_id;?>';</script>
                <?php
			}
		}
		?>
